...
function PerformanceEvaluationCreatePage() {
  ...
  const formik = useFormik<PerformanceEvaluationInterface>({
    initialValues: {
      ...
      score: 0,
      feedback: '',
      next_evaluation_date: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="next_evaluation_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Next Evaluation Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.next_evaluation_date ? new Date(formik.values?.next_evaluation_date) : null}
              onChange={(value: Date) => formik.setFieldValue('next_evaluation_date', value)}
            />
          </FormControl>
